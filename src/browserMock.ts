...
export function dispatchMockMessages(): void {
  ...
  // Connect the agent bridge to receive real hook events via WebSocket
  import('./agentBridge.js').then(({ connectAgentBridge }) => {
    connectAgentBridge();
    console.log('[BrowserMock] Agent bridge connected');
  });
}
